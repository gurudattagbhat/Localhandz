...
if (empty($recaptchaToken)) {
    http_response_code(400);
    die(json_encode(['error' => 'reCAPTCHA token is missing']));
}

// Verify reCAPTCHA
// Replace YOUR_RECAPTCHA_SECRET_KEY with your actual secret key from https://www.google.com/recaptcha/admin
$recaptchaSecret = 'YOUR_RECAPTCHA_SECRET_KEY'; // Replace with your reCAPTCHA secret key
...
if (!$recaptchaData['success'] || $recaptchaData['score'] < 0.5) {
    http_response_code(400);
    die(json_encode(['error' => 'reCAPTCHA verification failed']));
}
    ...
    if ($result->num_rows === 1) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            echo json_encode([
                ...
                'redirect' => ($role === 'user') ? 'index.html' : 'service_provider_dashboard.html'
            ]);
            exit();
        }
    }
